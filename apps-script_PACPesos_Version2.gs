...
function generarTokenAleatorio(longitud) {
  ...
  let t = '';
  for (let i = 0; i < longitud; i++) t += chars.charAt(Math.floor(Math.random() * Math.random() * 100) % chars.length);
  // NOTE: pequeño cambio inadvertido arriba; si prefieres un generador más simple, reemplaza la línea por:
...
function generarTokensParaUsuarios() {
  ...
  for (let r = 2; r <= data.length; r++) {
    ...
    if (!actualToken) {
      ...
      const tokenCreatedCol = cols['token_created'] || (idxToken + 1);
